...
def submit_answers():
    ...
    try:
        ...
        # Check if the user has already submitted answers for this survey
        username = session.get('username')
        existing_submission = db.answers.find_one({
            "survey_id": survey_id,
            # "submitted_by": username
        })
        
        if existing_submission:
            flash('You have already submitted answers for this survey.', 'error')
            return redirect(url_for('answer_questions'))
        ...
        for question_id, answer_values in answers_data.items():
            ...
            try:
                ...
                if question:
                    ...
                    # For checkbox questions, combine multiple answers into a single comma-separated string
                    if question_type == "checkbox" and len(answer_values) > 1:
                        combined_answer = ",".join(answer_values)
                        answers.append({
                            "survey_id": survey_id,
                        ...
                        # For each answer value in the list (single selection or open-ended)
...
def signup():
    if request.method == 'POST':
        try:
            ...
            existing_email = db.employee.find_one({"email": email})
            if existing_email:
                flash('This email is already registered. Please use a different email.', 'error')
                return redirect(url_for('signup'))
                
            # Check if username already exists
            existing_username = db.employee.find_one({"username": username})
...
def export_all_responses():
    try:
        ...
        # Create a dictionary to group responses by survey_id, submission_date, submission_time, and submitted_by
        grouped_responses = {}
        for response in all_responses:
            ...
            # submitted_by = response.get("submitted_by", "Unknown User")
        ...
        def generate_csv():
            ...
            for submission_data in grouped_responses.values():
                ...
                row = [
                    ...
                    # f'"{submission_data["submitted_by"]}"'
                ]
